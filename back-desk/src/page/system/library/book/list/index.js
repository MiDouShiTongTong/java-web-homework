...
export default connect(
  ...
  Form.create()(
    class SystemLibraryBookList extends React.Component {
      state = {
        columns: [
          { title: '图书名称', dataIndex: 'name' },
          { title: '图书分类名称', dataIndex: 'bookCategoryName' },
          ...
          { title: '最后修改日期', dataIndex: 'updatedAt' },
          {
            title: '操作', dataIndex: 'action', render: (text, record) => (
              <div className="data-source-operation-container">
                <Link to={`/system/library/book/operator/${record.id}`}>编辑</Link>
      ...
      refreshData = async () => {
        ...
        // 获取远程数据
        const result = await api.book.selectBookList({
      ...
      deleteData = (record) => {
        Modal.confirm({
          ...
          onOk: async () => {
            // loading
            this.setState({ loading: true });
            await api.book.deleteBookById(record.id);
            // 刷新表格数据
            this.refreshData();
          },
          onCancel() {
        ...
        return (
          <section className="operation-container">
            ...
            <section className="data-action-container">
              <Link to="/system/library/book/operator">
